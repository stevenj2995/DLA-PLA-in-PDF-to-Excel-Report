...
def _labels_of(document, profile: Profile | None):
    ...
    if not sections:
        return None, "", "", "DLA isn't found!", 0, 0
    ...
    if not label or not names:
        ...
        return None, "", "", (f"berkas memuat {total} {what}, dan profil belum " f"tahu mana yang milik kita"), total, 0
    ...
    if len(mine) == 1:
        ...
        note = (f"File memuat {total} {what}; diambil yang ditujukan ke "
...
def read_one(path: Path, profile: Profile, document=None) -> FileResult:
    ...
    if not document.has_text:
        result.reason = ("tidak ada teks yang bisa dibaca, mungkin perlu OCR")
        return result
...
def run(paths, *, profile_key: str | None = None, progress=None) -> BatchResult:
    ...
    if batch.scanned:
        batch.notes.append(Note(
            f"{len(batch.scanned)} PDF dibaca lewat OCR karena tidak punya teks",
            [f.name for f in batch.scanned], level="warn"))

    blank = [f for f in batch.done if f.missing]
    if blank:
        batch.notes.append(Note(
            f"{len(blank)} PDF tidak memuat sebagian parameter.",
...
def _grouped(files: list[FileResult], profile: Profile) -> list[Group]:
    ...
    for key in order:
        ...
        rows = [[f.values[i] for i in keep] + [f.extra.get(x, "") for x in extras] + [f.name]
                for f in members]
        groups.append(Group(headers=headers, rows=rows, files=members, caption=f"{len(rows)} {kind or 'dokumen'} - "
...
def _detect(paths: list[Path], batch: BatchResult) -> Profile | None:
    for path in paths:
        ...
        if found:
            return found
    batch.rejected = ("Perusahaan tidak dikenali!")
    return None

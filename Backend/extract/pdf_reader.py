...
def scanned_pages(document: Document) -> list[int]:
    return [i for i, page in enumerate(document.pages) if sum(len(l.strip()) for l in page.lines) < settings.SCANNED_PAGE_CHARS]
...
def read(path: str | Path, *, use_ocr: bool = True) -> Document:
    ...
    try:
        ...
        document.error = f"Gabisa dibuka: {e}"
    return document